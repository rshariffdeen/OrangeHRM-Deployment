...
printf "*******************************************\n\n";
sudo mkdir -p /data/consul/{data,config}
docker-compose -f docker-compose.yml up -d orangehrm_service_registry

printf "\n\nConfiguring Service Discovery and Service Registry\n";
printf "*******************************************\n\n";
docker cp ../scripts/consul/check-disk.sh orangehrm_service_discovery:/data/consul/scripts/check-disk.sh
docker cp ../config/consul/consul_check.json orangehrm_service_discovery:/data/consul/config/consul_check.json
docker kill -s HUP orangehrm_service_discovery
...
printf "*******************************************\n\n";
docker cp ../conf/nginx.conf  orangehrm_proxy:/etc/nginx/conf.d/default.conf
docker cp ../conf/orangehrm-includes.conf  orangehrm_proxy:/etc/nginx/includes/orangehrm-includes.conf
